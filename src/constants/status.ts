...
function labelled<Key extends string> (
  ...
  const entries = Object.entries(looks) as [Key, Omit<StatusDefinition, 'label'>][]

  return Object.fromEntries(entries.map(([key, look]) => [key, {
    ...look,
    get label () {
...
export const DEFAULT_ROLE_NAMES: DefaultRoleName[] = ['SUPERADMIN', 'ADMIN', 'MANAGER', 'VIEWER']
